...
Spree.ready(function($) {
  ...
  $('input[name^="options"], select[name^="options"]').each(function() {
      ...
      // Extract the option key and index from the input name
      var matches = name.match(/options\[(\w+)\]\[(\d+)\]/);
      if (matches && matches.length === 3) {
          var key = matches[1];
          var index = matches[2];

          // Create the options hash if it doesn't exist
          options[key] = options[key] || {};
          
          // Add the value to the options hash at the specified index
